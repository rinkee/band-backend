...
async function extractOrdersFromCommentsAI(
  ...
  const aiEndpoint = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-lite:generateContent?key=${aiApiKey}`;
      ...
      try {
        ...
        if (useAIResults && aiOrderResults.length > 0) {
          ...
          if (aiResult) {
            aiAnalyzed = true;
            console.log(
              `[주문 생성] AI 분석 결과 사용 - ${commentKey}: isOrder=${aiResult.isOrder}, reason=${aiResult.reason}`
            );
            ...
            if (aiResult.isOrder) {
              ...
              console.log(
                `[주문 생성] AI 판별: 주문 아님 - ${commentKey}: ${aiResult.reason}`
              );
              processingSummary.skippedMissingInfo++; // AI가 판별한 비주문으로 분류
              continue;
            }
          }
        }
        ...
        if (isProcessedAsOrder && representativeItem) {
          ...
          // AI 분석 정보를 sub_status에 추가
          if (representativeItem?.aiAnalyzed) {
            const aiInfo = `AI 분석: ${representativeItem.aiReason}`;
            finalSubStatus = finalSubStatus
              ? `${finalSubStatus}, ${aiInfo}`
              : aiInfo;
          }
          ...
          const orderData = {
            ...
            created_at: new Date().toISOString(),
            updated_at: new Date().toISOString(),
            // AI 분석 관련 메타데이터 (필요시 주석 해제)
            // ai_analyzed: representativeItem?.aiAnalyzed || false,
            // ai_reason: representativeItem?.aiReason || null,
          };
