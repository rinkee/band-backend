...
const requireAuth = (req, res, next) => {
  // authMiddleware가 먼저 실행되어 req.user를 설정한다고 가정
  console.log("\n--- [requireAuth 시작] ---");
  if (!req.user) {
...
const requireSelfOrAdmin = async (req, res, next) => {
  ...
  try {
    if (!req.user) {
      // <<< req.user 확인
      console.log(
        "[requireSelfOrAdmin] 실패: req.user 없음 (이전 미들웨어 문제?)"
      );
      return res.status(401).json({
        success: false,
    ...
    console.log(
      "  - tokenUserId:",
      tokenUserId,
      `(Type: ${typeof tokenUserId})`
    );
    console.log("  - role:", role, `(Type: ${typeof role})`);
    console.log(
      "  - targetUserId (from URL):",
      targetUserId,
    ...
    console.log(
      `[requireSelfOrAdmin] 조건 확인: isSelf=${isSelf}, isAdmin=${isAdmin}`
    );

    if (isSelf || isAdmin) {
      console.log(
        "[requireSelfOrAdmin] 통과: 권한 확인됨 (자기 자신 또는 관리자)"
      );
...
const requireActiveUser = async (req, res, next) => {
  ...
  try {
    ...
    console.log("[requireActiveUser] Supabase 조회 결과:", {
      userData,
      error,
    });
    if (error) {
      console.error(
        "[requireActiveUser] 실패: Supabase 사용자 조회 오류 -",
        error.message
      );
      return res.status(500).json({
        success: false,
    ...
    if (!userData) {
      // 혹시 모를 경우 대비
      console.error(
        `[requireActiveUser] 실패: Supabase에서 사용자 찾을 수 없음 - userId: ${userId}`
      );
      return res
        .status(404)
